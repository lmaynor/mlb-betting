...
@app.route("/backfill-f5-book", methods=["POST"])
def backfill_f5_book():
    import json
    from google.cloud import storage as gcs
    from mlb_core.config import GCS_BUCKET
    from mlb_core.odds.sgo import extract_f5_ml_odds
    ...
    with engine.connect() as conn:
        rows = conn.execute(text(
            "SELECT id, game_date, away_team, home_team FROM bets "
            "WHERE system = 'F5' AND book IS NULL AND kelly_triggered = true ORDER BY game_date"
        )).fetchall()
    ...
    for date_str, bets in sorted(by_date.items()):
        prefix = f"Odds/sgo/{date_str}/"
        blobs = sorted(b.name for b in bucket.list_blobs(prefix=prefix) if b.name.endswith(".json"))
        book_map = {}
        for snap in blobs:
            try:
                events = json.loads(bucket.blob(snap).download_as_text())
                ml = extract_f5_ml_odds(events)
                for info in ml.values():
                    a = resolve_team(info["away_team"])
                    h = resolve_team(info["home_team"])
                    bk = info.get("bookmaker")
                    if a and h and bk:
                        book_map[(a, h)] = bk
                if book_map:
                    break
        ...
        with engine.begin() as conn:
            for bet in bets:
                ...
                book = book_map.get(key) or book_map.get((resolve_team(bet.away_team or ""), resolve_team(bet.home_team or "")))
                if book:
                    conn.execute(text("UPDATE bets SET book = :book WHERE id = :id"), {"book": book, "id": bet.id})
                    updated += 1
                else:
                    skipped += 1
