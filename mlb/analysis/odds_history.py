...
# player_id is REQUIRED here: without it, all players' props on the same
# (market, game_pk, selection, line, book, snapshot_ts) collapse to one row.
...
def partition_dir(market: str, game_date: str) -> str:
    return f"{HISTORY_PREFIX}/market={market}/date={game_date}/"


def partition_path(market: str, game_date: str) -> str:
    """The canonical single-file name used by an append=False (full
    overwrite) write. append=True writes land in the same directory under
    their own part-{uuid}.parquet names -- see write_partition."""
    return f"{partition_dir(market, game_date)}part-0.parquet"


def _partition_files(market: str, game_date: str) -> list:
    from mlb_core import storage
    return storage.list_keys(partition_dir(market, game_date))
...
def write_partition(df, market: str, game_date: str, append: bool = False) -> int:
    ...
    missing columns, de-dups the INCOMING batch on DEDUP_KEYS. No-op for an
    empty frame.

    append=False (default): OVERWRITE the partition -- deletes any existing
      part file(s) for this (market, date) first, then writes a single fresh
      part-0.parquet. Used by historical re-ingest to REPLACE corrupt rows;
      the delete-first step matters so a maintenance rewrite can't leave
      stale append-mode files behind for read_history() to silently merge
      back in.
    append=True: writes this batch as its OWN new file alongside whatever's
      already in the partition directory -- does NOT read or rewrite
      existing data. Multiple snapshots/day ACCUMULATE as separate small
      files; read_history() is what merges + de-dups across all of a
      partition's files (once, at read time), not this function.

      This used to read the entire existing partition, concat, de-dup, and
      rewrite the whole thing on every single append -- fine for a few
      writes/day, but a partition touched dozens of times/day during a live
      slate (fast_alert_loop's */15 cadence) meant re-reading and
      re-writing an ever-growing file on every call, which is what made
      that job take 4-8 minutes/run. See
      docs/solutions/logic-errors/odds-history-append-write-amplification.md.

    Returns the number of rows WRITTEN by this call (post self-dedup of the
    incoming batch) -- NOT the partition's total row count across all files.
    Call read_history() (or coverage_report() for a cheap count-only view)
    for the partition's actual total."""
    import pandas as pd
    from mlb_core import storage
    ...
    if append:
        key = f"{partition_dir(market, game_date)}part-{uuid.uuid4().hex[:12]}.parquet"
    else:
        for existing in _partition_files(market, game_date):
            storage.delete(existing)
        key = partition_path(market, game_date)
    storage.write_bytes(_to_parquet_bytes(df), key)
    return len(df)
...
def _list_partition_dates(market: str) -> list:
    ...
    for key in storage.list_keys(f"{HISTORY_PREFIX}/market={market}/"):
        # .../market=<m>/date=<YYYY-MM-DD>/part-0.parquet (or part-{uuid}.parquet)
...
def read_history(market: str, since: str | None = None, until: str | None = None):
    ...
    A partition directory can hold multiple files -- one per write_partition
    append=True call (see its docstring). This is where they actually get
    merged and de-duped on DEDUP_KEYS, once, instead of on every write.
    Files are fetched concurrently (pure I/O wait, parallelizes cleanly) so
    a partition with many small append-files doesn't cost one round trip
    per file in series."""
    import pandas as pd
    from mlb_core import storage
    ...
    keys = [k for d in dates for k in _partition_files(market, d)]

    def _read_one(key):
        try:
            return pd.read_parquet(io.BytesIO(storage.read_bytes(key)))
        except Exception:  # noqa: BLE001 -- skip a missing/corrupt file
            return None

    frames = []
    if keys:
        with ThreadPoolExecutor(max_workers=min(16, len(keys))) as ex:
            frames = [f for f in ex.map(_read_one, keys) if f is not None]

    if not frames:
        return pd.DataFrame(columns=SCHEMA_COLUMNS)
    combined = pd.concat(frames, ignore_index=True)
    dedup_keys = [c for c in DEDUP_KEYS if c in combined.columns]
    # Best-effort "latest wins" on a genuine cross-file duplicate (the same
    # quote written more than once, e.g. a retried job): sort by ingested_at
    # first when present so drop_duplicates(keep="last") favors the
    # most-recently-ingested copy, matching the old merge-at-write-time
    # behavior. A true value conflict on an exact dedup-key match across
    # files is rare (idempotent retries carry identical values anyway).
    if "ingested_at" in combined.columns:
        combined = combined.sort_values("ingested_at", kind="stable")
    return (combined.drop_duplicates(subset=dedup_keys, keep="last")
