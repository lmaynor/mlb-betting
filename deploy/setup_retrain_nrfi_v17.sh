#!/usr/bin/env bash
# setup_retrain_nrfi_v17.sh — provision the Cloud Run Job for NRFI v17 full retrain.
#
...
#   - NRFI_Pro_System/data/model_features.csv exists in GCS
...
if gcloud run jobs describe "$JOB_NAME" --region="$REGION" --quiet >/dev/null 2>&1; then
  ...
  gcloud run jobs update "$JOB_NAME" \
    ...
    --command="python" \
    --args="-m,mlb.training.retrain_nrfi_v17" \
    --memory=2Gi \
    --cpu=2 \
    --task-timeout=1800 \
    --max-retries=1 \
...
echo "  gsutil ls gs://\${MLB_GCS_BUCKET}/NRFI_Pro_System/models/"
echo ""
echo "  Should show:"
echo "    xgb_halfinn_v17.json"
echo "    model_meta_v17.json"
echo "    archive/xgb_halfinn_v17.{ts}.json"
echo "    archive/model_meta_v17.{ts}.json"
echo ""
