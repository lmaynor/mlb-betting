...
describe('betting-api throws when BETTING_API_URL not set', () => {
  ...
  test('apiGetTodayPicks throws if BETTING_API_URL not set', async () => {
  ...
  test('apiGetStats throws if BETTING_API_URL not set', async () => {
...
describe('ResultPill', () => {
  ...
  test('win result has text-win class', () => {
    const { container } = render(<ResultPill result="win" />)
    expect(container.firstChild).toHaveClass('text-win')
  })

  test('loss result has text-loss class', () => {
    const { container } = render(<ResultPill result="loss" />)
    expect(container.firstChild).toHaveClass('text-loss')
  })
...
describe('migrate route contract', () => {
